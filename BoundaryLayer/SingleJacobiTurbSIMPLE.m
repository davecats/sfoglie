function [ f1,f2,f3,df_dT,df_dD,df_dCt, params,df_dU] = SingleJacobiTurbSIMPLE( DI,T,Ct,U,Vb,h,LTE,diffU,IsWake)
%SINGLEJACOBITURB    Version with simplified shear stress EQ!!!!!!!!!
%                   calculates the funtcionvalue and the derivates for Newton method
...
k=HS.*0.015./(1-Us).*(H-1).^3./H.^3 ;
CE= sqrt( k );
dCE_dH = ( 3*k./(H-1) -k./H ) ./(2.*CE);
dCE_dHS = ( k./HS ) ./(2.*CE);
dCE_dUs = ( k./(1-Us) ) ./(2.*CE);
dCE_dT = dCE_dH.*dH_dT  + dCE_dHS.*dHS_dT + dCE_dUs.*dUs_dT ;
...
dCD_dCf= Us;
dCD_dCt= 2*Ct.*(1-Us);
dCD_dT =  dCD_dUs.*dUs_dT + dCD_dCf.*dCf_dT ;
dCD_dD =  dCD_dUs.*dUs_dD + dCD_dCf.*dCf_dD ;
...
if diffU
    ...
    dUs_dU = dUs_dHS.*dHS_dU;
    dCE_dU = dCE_dHS.*dHS_dU + dCE_dUs.*dUs_dU ;
    dCD_dU = dCD_dUs.*dUs_dU + dCD_dCf.*dCf_dU ;
end
...
% momentum thickness T
df3_dT2 = 4.2*h.*dCEM_dT2  - 2*dDelM_dT2.*Ctlog;
% displacement thickness DI
df3_dD2 = 4.2*h.*dCEM_dD2  - 2*dDelM_dD2.*Ctlog;
% shear stress c
df3_dCt2= -4.2.*h.*dCtM_dCt2 - 2*DelM./Ct2 ;  
% Boundary layer edge velocity U
if diffU
    df3_dU2 = 4.2*h.*dCEM_dU2 ; 
